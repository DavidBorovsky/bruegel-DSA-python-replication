...
# ========================================================================================= #
    def __init__(self, 
        ...
        self.stochastic_projection_period = self.end_year - stochastic_start_year
        if shock_frequency == 'quarterly':
            self.draw_period = self.stochastic_projection_period * 4 
        elif shock_frequency == 'annual':
            self.draw_period = self.stochastic_projection_period
...
# ========================================================================================= #
    ...
    def _aggregate_shocks_quarterly(self):
        ...
        self.shocks_sim_grouped = self.shocks_sim_draws.reshape((self.N, self.stochastic_projection_period, 4, self.num_variables))
        ...
        try:
            ...
            exr_eur_shocks = np.zeros((self.N, self.stochastic_projection_period))
        try:
            exr_usd_shocks = np.sum(self.shocks_sim_grouped[:, :, :, -5], axis=2)
        except:
            exr_usd_shocks = np.zeros((self.N, self.stochastic_projection_period))
        ...
        self.long_term_interest_rate_shocks = np.zeros((self.N, self.stochastic_projection_period))

        # Iterate over each year
        for t in range(1, self.stochastic_projection_period+1):
            q = t * 4
        ...
        if self.stochastic_projection_period > post_adjustment_period:
    ...
    def _aggregate_shocks_annual(self):
        ...
        """
        # Reshape the shocks
        self.shocks_sim_grouped = self.shocks_sim_draws.reshape((self.N, self.stochastic_projection_period, self.num_variables))
    ...
    def _combine_shocks_baseline(self):
        ...
        d_sim = np.zeros([self.N, self.stochastic_projection_period+1])  # Debt to GDP ratio
        exr_eur_sim = np.zeros([self.N, self.stochastic_projection_period+1])  # EUR exchange rate
        exr_usd_sim = np.zeros([self.N, self.stochastic_projection_period+1])  # USD exchange rate
        iir_sim = np.zeros([self.N, self.stochastic_projection_period+1])  # Implicit interest rate
        ng_sim = np.zeros([self.N, self.stochastic_projection_period+1])  # Nominal GDP growth
        pb_sim = np.zeros([self.N, self.stochastic_projection_period+1])  # Primary balance
        sf_sim = np.zeros([self.N, self.stochastic_projection_period+1])  # Stock flow adjustment

        # Call the Numba JIT function with converted self variables
        combine_shocks_baseline_jit(
            N=self.N, 
            stochastic_projection_period=self.stochastic_projection_period, 
...
# ========================================================================================= #
    ...
    def fanchart(self, save_as=False, show=True, variable='d', periods=5):
        ...
        ax.fill_between(years[-self.stochastic_projection_period-1:-self.stochastic_projection_period-1+periods+1], self.pcts_dict[10], self.pcts_dict[90], label='10th-90th percentile')
        ax.fill_between(years[-self.stochastic_projection_period-1:-self.stochastic_projection_period-1+periods+1], self.pcts_dict[20], self.pcts_dict[80], label='20th-80th percentile')
        ax.fill_between(years[-self.stochastic_projection_period-1:-self.stochastic_projection_period-1+periods+1], self.pcts_dict[30], self.pcts_dict[70], label='30th-70th percentile')
        ax.fill_between(years[-self.stochastic_projection_period-1:-self.stochastic_projection_period-1+periods+1], self.pcts_dict[40], self.pcts_dict[60], label='40th-60th percentile')
        ax.plot(years[-self.stochastic_projection_period-1:-self.stochastic_projection_period-1+periods+1], self.pcts_dict[50], alpha=1, ls='-', color='black', label='median')
        ...
        for pct in self.pcts_dict:
            df_pct = pd.DataFrame({'year': years[-self.stochastic_projection_period-1:-self.stochastic_projection_period-1+periods+1], f'p{pct}': self.pcts_dict[pct]})
...
# ========================================================================================= #
    ...
    def find_deficit_prob(self):
        ...
        self.stochastic_projection_period = self.projection_period - self.adjustment_start
        if self.shock_frequency == 'quarterly':
            self.draw_period = self.stochastic_projection_period * 4 
        else:
            self.draw_period = self.stochastic_projection_period
        ...
        # Simulate deficit
        self.ob_sim = np.zeros([self.N, self.stochastic_projection_period+1])
        self.ob_sim[:, 0] = self.ob[-(self.stochastic_projection_period+1)]
    ...
    def _simulate_deficit(self):
        ...
        simulate_deficit_jit(
            N=self.N, stochastic_projection_period=self.stochastic_projection_period, pb_sim=self.pb_sim, iir_sim=self.iir_sim, ng_sim=self.ng_sim, d_sim=self.d_sim, ob_sim=self.ob_sim
            )
...
@jit(nopython=True)
def combine_shocks_baseline_jit(N, stochastic_projection_period, shocks_sim, exr_eur, exr_usd, iir, ng, pb, sf, d, d_sim, exr_eur_sim, exr_usd_sim, iir_sim, ng_sim, pb_sim, sf_sim):
    """
    ...
    # Combines shocks with baseline variables for periods after adjustment end, so from - stochastic_projection_period onward
    for n in range(N):
        exr_eur_sim[n, 1:] = exr_eur[-stochastic_projection_period:] + shocks_sim[n, 0] 
        exr_usd_sim[n, 1:] = exr_usd[-stochastic_projection_period:] + shocks_sim[n, 1]
        iir_sim[n, 1:] = iir[-stochastic_projection_period:] + shocks_sim[n, 2]
        ng_sim[n, 1:] = ng[-stochastic_projection_period:] + shocks_sim[n, 3]
        pb_sim[n, 1:] = pb[-stochastic_projection_period:] + shocks_sim[n, 4]
    
    # Set values for stock-flow adjustment 
    sf_sim[:, 1:] = sf[-stochastic_projection_period:]

    # Set the starting values t0
    d_sim[:, 0] = d[-stochastic_projection_period-1]
    exr_eur_sim[:, 0] = exr_eur[-stochastic_projection_period-1]
    exr_usd_sim[:, 0] = exr_usd[-stochastic_projection_period-1]
    iir_sim[:, 0] = iir[-stochastic_projection_period-1]
    ng_sim[:, 0] = ng[-stochastic_projection_period-1]
    pb_sim[:, 0] = pb[-stochastic_projection_period-1]

@jit(nopython=True)
def simulate_debt_jit(N, stochastic_projection_period, D_share_domestic, D_share_eur, D_share_usd, d_sim, iir_sim, ng_sim, exr_eur_sim, exr_usd_sim, pb_sim, sf_sim):
    """
    Simulate the debt-to-GDP ratio using the baseline variables and the shocks.
    """
    for n in range(N):
        for t in range(1, stochastic_projection_period+1):
...
@jit(nopython=True)
def simulate_deficit_jit(N, stochastic_projection_period, pb_sim, iir_sim, ng_sim, d_sim, ob_sim):
    """
    Simulate the fiscal balance ratio using the baseline variables and the shocks.
    """
    for n in range(N):
